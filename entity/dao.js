...
function async updateMany(sa,entityCode,entitys){
    ...
    //校验权限; --数据库中获取看是否成功;--
    let etCount = countBySql(sa,entityCode,ustr.ls2sqlWhere(key,ids));
...
//不提供insertBySql;
function async insert(sa,entityCode,entity){
...
function async insertMany(sa,entityCode,entitys){
...
//validate
//require/number/int/datetime/email/chs/eng/max/min/between/enum/ds
//
        ...
        for(let et of entitys){         
            if(val==null||val==undefined){      
                ...
                if(fldType=='varchar'){ //varchar,int,double,datetime,
                    if(val&&a.FldSize&&.length>a.FldSize){
                        throw Err.getErrValidate900(a.FldName+","+a.FldLabel+",不能超过长度"+a.FldSize); //超长,或自动截断?
                    }
                }
                else if(fldType=='int'){ //varchar,int,double,datetime,
                    if(!ustr.isNumber(val)) 
                        throw Err.getErrValidate900(a.FldName+","+a.FldLabel+",必须为数字!" );
                }
                else if(fldType=='double'){ //varchar,int,double,datetime,
                    if(!ustr.isNumber(val)) 
                        throw Err.getErrValidate900(a.FldName+","+a.FldLabel+",必须为数字!" );
                }
                else if(fldType=='datetime'){ //varchar,int,double,datetime,
                    if(!ustr.isDate(val)) 
